# Knowledge Base glossary authoring guide
...
```text
blog-source/content/knowledge-base/glossary/<term>/index.md
```
...
- Do not hand-edit `prax/**`; it is generated by `scripts/import_prax_docs.py`.
- Do not hand-edit `blog/**`; regenerate it from `blog-source/` with `make blog`
  or `make ci`.
- Treat `deep-dives/**` and section `_index.md` files as separate content types.
  ...
  mechanically.
- The fuller figure and accessibility standard lives at
  `../agent-skill-documents/RESEARCH_NOTE_WRITEUP.md` relative to the repository
  root. Read its relevant sections when adding or materially changing a figure.
  (Posts have their own pointer to that guide at
  `blog-source/content/posts/AGENTS.md`; this glossary guide stays separate.)
