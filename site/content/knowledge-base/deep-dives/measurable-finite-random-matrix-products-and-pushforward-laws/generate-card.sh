...
source_svg="$script_dir/two-outcome-product-law-ledger.svg"
...
generate() {
  output="$1"
  work_dir="$(mktemp -d "/tmp/measurable-finite-products-card.XXXXXX")"
...
if test "$#" -gt 0 && test "$1" = "--verify"; then
  verification_file="$(mktemp "/tmp/measurable-finite-products-card-verify.XXXXXX")"
