...
source_svg="$script_dir/four-outcome-product-ledger.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/finite-product-fields-card-fit.XXXXXX)"
  rsvg-convert -w 976 -h 630 -b '#f7f1e7' -o "$fit" "$source_svg"
...
if test "$#" -eq 1 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/finite-product-fields-card.XXXXXX)"
  generate "$temporary"
