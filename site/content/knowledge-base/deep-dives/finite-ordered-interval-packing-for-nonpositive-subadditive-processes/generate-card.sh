...
source_svg="$script_dir/leftmost-greedy-packing-ledger.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/finite-ordered-packing-card-fit.XXXXXX.png)"
...
if test "$#" -eq 1 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/finite-ordered-packing-card.XXXXXX.png)"
  generate "$temporary"
