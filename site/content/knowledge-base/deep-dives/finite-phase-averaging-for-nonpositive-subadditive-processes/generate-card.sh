...
source_svg="$script_dir/three-phase-negative-orbit-ledger.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/finite-phase-averaging-card-fit.XXXXXX.png)"
...
if test "$#" -eq 1 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/finite-phase-averaging-card.XXXXXX.png)"
  generate "$temporary"
