...
source_svg="$script_dir/two-by-two-hermitian-congruence-ledger.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/hermitian-symmetry-card-fit.XXXXXX)"
...
if test "$#" -eq 1 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/hermitian-symmetry-card.XXXXXX)"
  generate "$temporary"
