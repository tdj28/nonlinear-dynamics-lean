...
source_svg="$script_dir/two-atom-cap-and-ratio-ledger.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/rmt31-all-length-card-fit.XXXXXX.png)"
...
if test "$#" -eq 1 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/rmt31-all-length-card.XXXXXX.png)"
  generate "$temporary"
