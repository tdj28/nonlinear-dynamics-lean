...
source_svg="$script_dir/four-cycle-partial-sums-and-hopf-event.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/finite-hopf-maximal-card-fit.XXXXXX.png)"
...
if test "$#" -eq 1 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/finite-hopf-maximal-card.XXXXXX.png)"
  generate "$temporary"
