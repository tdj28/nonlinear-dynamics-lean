...
source_svg="$script_dir/conditional-expectation.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/conditional-expectation-fit.XXXXXX)"
  rsvg-convert -w 966 -h 630 -b '#f7f1e7' -o "$fit" "$source_svg"
...
if test "$#" -gt 0 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/conditional-expectation-card.XXXXXX)"
  generate "$temporary"
