...
source_svg="$script_dir/coboundary-endpoint-cancellation.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/koopman-coboundary-fit.XXXXXX)"
...
if test "$#" -gt 0 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/koopman-coboundary-card.XXXXXX)"
  generate "$temporary"
