...
source_svg="$script_dir/finite-orbit-visit-count.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/finite-orbit-visit-count-fit.XXXXXX)"
...
if test "$#" -gt 0 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/finite-orbit-visit-count-card.XXXXXX)"
  generate "$temporary"
