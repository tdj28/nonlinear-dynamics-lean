...
source_svg="$script_dir/weak-type-one-one-maximal-bound.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/weak-type-one-one-maximal-bound-fit.XXXXXX)"
  rsvg-convert -w 900 -h 630 -b '#f7f1e7' \
    -o "$fit" "$source_svg"
...
if test "$#" -gt 0 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/weak-type-one-one-maximal-bound-card.XXXXXX)"
  generate "$temporary"
