...
source_svg="$script_dir/ordered-packing-on-a-finite-horizon.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/ordered-interval-packing-fit.XXXXXX)"
  rsvg-convert -w 788 -h 630 -b '#f7f1e7' -o "$fit" "$source_svg"
...
if test "$#" -gt 0 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/ordered-interval-packing-card.XXXXXX)"
  generate "$temporary"
