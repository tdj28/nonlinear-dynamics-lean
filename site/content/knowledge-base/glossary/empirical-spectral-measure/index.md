...
## The current measurability seam
...
The remaining input is measurability of each ordered eigenvalue coordinate as
a function of the matrix. Pinned Mathlib supplies the algebraic ordered
eigenvalues but does not supply the continuity or measurability theorem needed
here. The project therefore proves only conditional interfaces:
...
Read the hypothesis as an open proof obligation, not as documentation of an
already available instance. The same condition appears in the ambient
observable and GUE pushforward bridge.
...
It does **not** prove coordinatewise eigenvalue measurability or continuity.
Consequently it does not yet construct an unconditional GUE law on empirical
spectral measures. It also proves no eigenvalue density, semicircle law,
concentration estimate, unfolding procedure, spacing statistic, universality
result, or large-dimension limit.
...
ascent. The {{< refterm "finite-matrix-trace-moment" "finite matrix trace moment" >}}
