...
source_svg="$script_dir/event-membership-is-not-existence.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/birkhoff-convergence-event-fit.XXXXXX)"
  rsvg-convert -w 875 -h 630 -b '#f7f1e7' \
    -o "$fit" "$source_svg"
...
if test "$#" -gt 0 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/birkhoff-convergence-event-card.XXXXXX)"
  generate "$temporary"
