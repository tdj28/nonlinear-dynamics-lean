...
source_svg="$script_dir/normalization-ledger-trace.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/normalization-convention-fit.XXXXXX)"
  rsvg-convert -w 822 -h 630 -b '#f7f1e7' -o "$fit" "$source_svg"
...
if test "$#" -eq 1 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/normalization-convention-card.XXXXXX)"
  generate "$temporary"
