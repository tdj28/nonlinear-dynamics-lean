...
source_svg="$script_dir/independence-factorization.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/independence-fit.XXXXXX)"
  rsvg-convert -w 906 -h 630 -b '#f7f1e7' -o "$fit" "$source_svg"
...
if test "$#" -eq 1 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/independence-card.XXXXXX)"
  generate "$temporary"
