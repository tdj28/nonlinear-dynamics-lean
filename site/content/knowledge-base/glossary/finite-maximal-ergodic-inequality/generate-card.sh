...
source_svg="$script_dir/finite-maximal-event.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/finite-maximal-ergodic-inequality-fit.XXXXXX)"
...
if test "$#" -gt 0 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/finite-maximal-ergodic-inequality-card.XXXXXX)"
  generate "$temporary"
