...
source_svg="$script_dir/invariant-sigma-algebra.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/invariant-sigma-algebra-fit.XXXXXX)"
...
if test "$#" -gt 0 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/invariant-sigma-algebra-card.XXXXXX)"
  generate "$temporary"
