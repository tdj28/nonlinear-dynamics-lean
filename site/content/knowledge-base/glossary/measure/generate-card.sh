...
source_svg="$script_dir/three-atom-measure.svg"
checked="$script_dir/measure-card.png"
fit=""
temporary=""
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/measure-card-fit.XXXXXX)"
...
if test "$#" -eq 1 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/measure-card.XXXXXX)"
  generate "$temporary"
  cmp -s "$temporary" "$checked" || {
    echo "measure-card.png is stale; regenerate it" >&2
    exit 1
  }
  echo "verified measure-card.png"
  exit 0
fi
