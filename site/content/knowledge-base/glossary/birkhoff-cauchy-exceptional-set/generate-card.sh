...
source_svg="$script_dir/birkhoff-cauchy-exceptional-set.svg"
...
generate() {
  output="$1"
  fit="$(mktemp /tmp/birkhoff-cauchy-exceptional-set-fit.XXXXXX)"
...
if test "$#" -gt 0 && test "$1" = "--verify"; then
  temporary="$(mktemp /tmp/birkhoff-cauchy-exceptional-set-card.XXXXXX)"
  generate "$temporary"
