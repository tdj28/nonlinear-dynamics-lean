# Research Note posts: read the writeup skill first
...
- Applies to all of `blog-source/content/posts/**`.
- The knowledge-base glossary has its **own, separate** guide:
  `blog-source/content/knowledge-base/AGENTS.md`. Keep them separate; do not
  merge glossary rules into post rules or vice versa.
- Hugo ignores `AGENTS.md` files (`ignoreFiles` in `hugo.yaml`), so this file
  never renders on the site.

# Research Note Writeup

Status: **normative style and process guide for Praxagent Research Notes**
